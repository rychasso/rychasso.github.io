export const nouns = {
};

export const actions = {
};

export const menu = {
  'menu.about': 'Обо мне',
  'menu.portfolio': 'Портфолио',
  'menu.tag.characters': 'Персонажи',
  'menu.tag.illustrations': 'Иллюстрации',
  'menu.first-name': 'Алёна',
  'menu.last-name': 'Рычагова',
};

export const projects = {
  'project.THE-MIRACLE-BEAST.title': 'Чюдо-Юдо',
  'project.THE-MIRACLE-BEAST.description': 'Иллюстрации подготовлены по&nbsp;стихотворению &laquo;Чудо&mdash;Юдо&raquo; детского писателя Елены Бурак {$link}',
  'project.THE-MIRACLE-BEAST.prepare-description': 'Подготовка персонажа',
  'project.THE-MIRACLE-BEAST.steps-description': 'Определение динамики, раскадровка, цвет',
  'project.THE-MIRACLE-BEAST.templates-description': 'Подготовка экскизов',
  'project.THE-MIRACLE-BEAST.results-description': 'Финальные работы',

  'project.Childhood-Trading.title': 'Торговля',
  'project.Karaoke.title': 'Караоке',
  'project.Attraction.title': 'Аттракцион',
  'project.The-Dreamers.title': 'Мечтатели',
  'project.Kindergarten.title': 'Однажды в детском саду',
  'project.Lessons-Outside.title': 'Учёба на свежем воздухе',
  'project.Tailwind.title': 'Попутный ветер',
  'project.Storm.title': 'Шторм',
  'project.Masha-And-Seeds.title': 'Маша и семена',
  'project.Semen-Mikhailovich-And-Vanya.title': 'Семён Михаилович и Ваня',
  'project.Alenushka.title': 'Алёнушка',
  'project.Fox-Family.title': 'Семейный портрет',
  'project.Misha-The-President.title': 'Президент Миша',
  'project.Girl-With-Peaches.title': 'Девочка с персиками',
  'project.Mona-Lisa.title': 'Мона Лиза',
  'project.Frida-Kahlo.title': 'Голубка Фрида',
  'project.Upside-Down-Magazine.title': 'Вверх-тормашками',
  'project.Spring-Animals.title': 'Рассвет весны',
  'project.Vasily-The-Cat.title': 'Один день из жизни Василия',
  'project.Winter-Street.title': 'Новогодняя улица',
  'project.Lolita-Grandma.title': 'Бабушка Лолита',
  'project.Pizzeria-The-Bo.title': 'Мистер Бо',
  'project.Bitterly.title': 'Горько',
  'project.Shaggy-Creeper.title': 'Мохнатая ползучка',
  'project.Emperor.title': 'Император',
};

const locale = {
  ...nouns,
  ...actions,
  ...menu,
  ...projects,
};

export default locale;
